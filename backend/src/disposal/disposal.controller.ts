...
export class DisposalController {
    ...
    @Post()
    @Roles(Role.ADMIN, Role.MANAGER, Role.MAINTENANCE)
    async create(@TenantId() tenantId: string, @CurrentUser('id') userId: string, @Body() dto: CreateDisposalDto) {
        return this.disposalService.create(tenantId, userId, dto);
    }

    @Post(':id/approve')
    @Roles(Role.MANAGER, Role.ADMIN)
    async approve(@TenantId() tenantId: string, @Param('id') id: string, @CurrentUser('id') userId: string) {
        return this.disposalService.approve(tenantId, id, userId);
    }

    @Post(':id/complete')
    @Roles(Role.ADMIN)
    async complete(@TenantId() tenantId: string, @Param('id') id: string, @Body() dto: CompleteDisposalDto) {
