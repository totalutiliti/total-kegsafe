...
export class DisposalService {
    ...
    async approve(tenantId: string, id: string, userId: string) {
        ...
        if (disposal.status !== DisposalStatus.PENDING_APPROVAL) {
            throw new BusinessException('DISPOSAL_INVALID_STATUS', 'Disposal is not pending approval');
        }
    ...
    async complete(tenantId: string, id: string, data: { destination: string; scrapValue?: number; notes?: string }) {
        const disposal = await this.findById(tenantId, id);
        if (disposal.status !== DisposalStatus.APPROVED) {
            throw new BusinessException('DISPOSAL_NOT_APPROVED', 'Disposal must be approved first');
        }
