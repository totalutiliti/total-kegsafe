...
export class StructuredLogger implements LoggerService {
    ...
    log(message: string, context?: LogContext | string) {
        this.writeLog('info', message, typeof context === 'string' ? undefined : context);
    }

    error(message: string, trace?: string, context?: LogContext | string) {
        this.writeLog('error', message, { ...(typeof context === 'string' ? {} : context), stack: trace });
    }

    warn(message: string, context?: LogContext | string) {
        this.writeLog('warn', message, typeof context === 'string' ? undefined : context);
    }

    debug(message: string, context?: LogContext | string) {
        if (process.env.NODE_ENV !== 'production') {
            this.writeLog('debug', message, typeof context === 'string' ? undefined : context);
        }
    }

    verbose(message: string, context?: LogContext | string) {
        this.writeLog('verbose', message, typeof context === 'string' ? undefined : context);
    }
