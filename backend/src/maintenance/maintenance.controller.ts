...
export class MaintenanceController {
    ...
    async findAllOrders(
        ...
        return this.maintenanceService.findAllOrders(tenantId, { status, page, limit });
    }
    ...
    async createOrder(@TenantId() tenantId: string, @Body() dto: CreateMaintenanceOrderDto) {
        return this.maintenanceService.createOrder(tenantId, dto);
    }

    @Post('checklist')
    @Roles(Role.MAINTENANCE, Role.ADMIN)
    async registerChecklist(@TenantId() tenantId: string, @CurrentUser('id') userId: string, @Body() dto: CreateChecklistDto) {
        return this.maintenanceService.registerChecklist(tenantId, userId, dto);
    }

    @Post('triage')
    @Roles(Role.MAINTENANCE, Role.LOGISTICS, Role.ADMIN)
    async registerTriage(@TenantId() tenantId: string, @CurrentUser('id') userId: string, @Body() dto: CreateTriageDto) {
