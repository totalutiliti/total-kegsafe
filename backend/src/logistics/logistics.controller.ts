...
export class LogisticsController {
    ...
    async expedition(@TenantId() tenantId: string, @CurrentUser('id') userId: string, @Body() dto: ExpeditionDto) {
        return this.logisticsService.expedition(tenantId, userId, dto);
    }

    @Post('delivery')
    @Roles(Role.LOGISTICS, Role.ADMIN)
    async delivery(@TenantId() tenantId: string, @CurrentUser('id') userId: string, @Body() dto: DeliveryDto) {
        return this.logisticsService.delivery(tenantId, userId, dto);
    }

    @Post('collection')
    @Roles(Role.LOGISTICS, Role.ADMIN)
    async collection(@TenantId() tenantId: string, @CurrentUser('id') userId: string, @Body() dto: CollectionDto) {
        return this.logisticsService.collection(tenantId, userId, dto);
    }

    @Post('reception')
    @Roles(Role.LOGISTICS, Role.ADMIN)
    async reception(@TenantId() tenantId: string, @CurrentUser('id') userId: string, @Body() dto: ReceptionDto) {
