...
export class AlertService {
    ...
    async findAll(tenantId: string, query?: { page?: number; limit?: number; type?: string; resolved?: boolean | string }) {
        ...
        const resolvedBool = query?.resolved !== undefined
    ...
    async acknowledge(tenantId: string, id: string, userId: string) {
        ...
        return this.prisma.alert.update({
            where: { id },
            data: { acknowledgedAt: new Date(), acknowledgedById: userId, status: 'ACKNOWLEDGED' },
        });
    }
    ...
    async resolve(tenantId: string, id: string, userId: string, resolution?: string) {
        ...
        return this.prisma.alert.update({
            where: { id },
            data: { resolvedAt: new Date(), resolvedById: userId, resolutionNotes: resolution, status: 'RESOLVED' },
        });
    }
    ...
    async getAlertCounts(tenantId: string) {
        const [total, pending, critical] = await Promise.all([
            ...
            this.prisma.alert.count({ where: { tenantId, resolvedAt: null, acknowledgedAt: null } }),
            this.prisma.alert.count({ where: { tenantId, resolvedAt: null, priority: 'CRITICAL' } }),
        ]);
