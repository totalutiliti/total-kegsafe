    ...
    return (
        <RoleGuard allowedRoles={['ADMIN', 'MANAGER']}>
            <div className="space-y-6">
                <div>
                    <h1 className="text-2xl font-bold text-white">Descarte</h1>
                    <p className="text-sm text-zinc-400 mt-1">Gestão de baixa patrimonial</p>
                </div>
                ...
                {suggestions.length > 0 && (
                    <Card className="border-amber-500/20 bg-amber-500/5">
                        <CardContent className="p-5">
                            ...
                            <p className="text-xs text-zinc-400 mb-3">Barris com custo de manutenção superior a 60% do valor de aquisição:</p>
                            <div className="space-y-2">
                                {suggestions.slice(0, 5).map((barrel: any) => (
                                    <div key={barrel.id} className="flex items-center justify-between rounded-lg border border-zinc-800 bg-zinc-900/50 p-3">
                                        <div>
                                            <span className="text-sm font-medium text-zinc-200">{barrel.internalCode}</span>
                                            <span className="ml-3 text-xs text-zinc-500">Manutenção: R$ {Number(barrel.totalMaintenanceCost).toFixed(2)}</span>
                                        </div>
                    ...
                    {disposals.length === 0 ? (
                        <Card className="border-zinc-800 bg-zinc-900/50">
                            <CardContent className="flex flex-col items-center justify-center py-12">
                                <Trash2 className="h-12 w-12 text-zinc-700 mb-3" />
                                <p className="text-zinc-400">Nenhuma solicitação de descarte</p>
                            </CardContent>
                        </Card>
                    ) : disposals.map((d) => {
                        const sc = statusConfig[d.status] || statusConfig.PENDING_APPROVAL;
                        return (
                            <Card key={d.id} className="border-zinc-800 bg-zinc-900/50">
                                <CardContent className="flex items-center gap-4 p-5">
                                    ...
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-1">
                                            <span className="text-sm font-medium text-zinc-200">{d.barrel?.internalCode}</span>
                                            <Badge variant="outline" className={`text-[10px] ${sc.color}`}>{sc.label}</Badge>
                                        </div>
                                        <p className="text-xs text-zinc-500 truncate">{d.reason}</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-sm text-zinc-300">TCO: R$ {Number(d.tcoAccumulated).toFixed(2)}</p>
                                    </div>
                                </CardContent>
                            </Card>
                        );
