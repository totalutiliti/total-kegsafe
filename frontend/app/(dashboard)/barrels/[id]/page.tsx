    ...
    if (!barrel) {
        return (
            <div className="flex flex-col items-center justify-center py-24">
                ...
                <p className="text-zinc-400">Barril não encontrado</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-4">
                <Button variant="ghost" size="icon" onClick={() => router.back()} className="text-zinc-400 hover:text-zinc-200">
                    <ArrowLeft className="h-5 w-5" />
                </Button>
                <div className="flex-1">
                    <div className="flex items-center gap-3">
                        <h1 className="text-2xl font-bold text-white">{barrel.internalCode}</h1>
                        <Badge variant="outline" className={`${sc.color}`}>{sc.label}</Badge>
                    </div>
                    <p className="text-sm text-zinc-500 mt-0.5">QR: {barrel.qrCode} • {barrel.manufacturer} • {barrel.capacityLiters}L</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                <Card className="border-zinc-800 bg-zinc-900/50">
                    <CardContent className="p-4 text-center">
                        <p className="text-2xl font-bold text-white">{barrel.totalCycles}</p>
                        <p className="text-[11px] text-zinc-500">Ciclos Totais</p>
                    </CardContent>
                </Card>
                <Card className="border-zinc-800 bg-zinc-900/50">
                    <CardContent className="p-4 text-center">
                        <p className="text-2xl font-bold text-white">{barrel.capacityLiters}L</p>
                        <p className="text-[11px] text-zinc-500">Capacidade</p>
                    </CardContent>
                </Card>
                <Card className="border-zinc-800 bg-zinc-900/50">
                    <CardContent className="p-4 text-center">
                        <p className="text-2xl font-bold text-white">{barrel.tareWeightKg}kg</p>
                        <p className="text-[11px] text-zinc-500">Peso Tara</p>
                    </CardContent>
                </Card>
                <Card className="border-zinc-800 bg-zinc-900/50">
                    <CardContent className="p-4 text-center">
                        <p className="text-2xl font-bold text-white">R$ {Number(barrel.acquisitionCost || 0).toFixed(0)}</p>
                        <p className="text-[11px] text-zinc-500">Custo Aquisição</p>
                    </CardContent>
                </Card>
            </div>
                            ...
                            <Card key={cycle.id} className={`border ${hc.bg}`}>
                                <CardContent className="p-4">
                                    <div className="flex items-center justify-between mb-3">
                                        <h3 className="text-sm font-medium text-zinc-200 truncate pr-2">{config.name || 'Componente'}</h3>
                                        <Badge variant="outline" className={`text-[10px] ${hc.bg} ${hc.color}`}>{hc.label}</Badge>
                                    </div>
                                    {/* Progress bar */}
                                    <div className="w-full h-2 rounded-full bg-zinc-800 mb-3">
                                        <div
                                    ...
                                    <div className="grid grid-cols-2 gap-2 text-xs">
                                        <div>
                                            <span className="text-zinc-500">Ciclos:</span>
                                            <span className="ml-1 text-zinc-300">{cycle.cyclesSinceLastService}/{config.maxCycles}</span>
                                        </div>
                                        <div>
                    ...
                    <CardContent className="p-5">
                        {timeline.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-8">
                                <Truck className="h-10 w-10 text-zinc-700 mb-2" />
                                            ...
                                            <div key={event.id || i} className="relative flex items-start gap-4 pl-1">
                                                ...
                                                <div className="flex-1 min-w-0 pt-1">
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-sm font-medium text-zinc-200">{event.inputType}</span>
                                                        {event.client && (
                                                            <span className="text-xs text-zinc-500">→ {event.client.tradeName || event.client.name}</span>
                                                        )}
                                                    </div>
                                                    <div className="flex items-center gap-3 mt-0.5">
                                                        ...
                                                        {event.user && (
                                                            <span className="text-xs text-zinc-600">por {event.user.name}</span>
                                                        )}
                                                    </div>
                                                    {event.notes && <p className="text-xs text-zinc-500 mt-1">{event.notes}</p>}
                                                </div>
                                            </div>
                                        );
                                    })}
